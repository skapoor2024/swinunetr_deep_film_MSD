...
parser = argparse.ArgumentParser(description="Give the file location of the the dataset list")
parser.add_argument('--organ_file_dir',type=str,required=False,help='Path to the organ list text file')
parser.add_argument('--num_workers',type=int,default=8,required=False,help='Num of cores for the workers')

args = parser.parse_args()

ORGAN_DATASET_DIR = '/blue/kgong/s.kapoor/language_guided_segmentation/CLIP-Driven-Universal-Model_MSD_only/'
ORGAN_LIST = 'dataset/dataset_list/PAOT_10_inner_train.txt'

if args.organ_file_dir is not None:
    ORGAN_LIST = args.organ_file_dir

NUM_WORKER = args.num_workers
#choosing only MSD ones
NUM_CLASS = 9
TRANSFER_LIST = ['10_03','10_06','10_07','10_08','10_09','10_10'] #need to get this from the paot text
## full list
#TRANSFER_LIST = ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10_03', '10_06', '10_07', '10_08', '10_09', '10_10', '12', '13']
...
#Template for MSD with new labels.
...
def generate_label(input_lbl, num_classes, name, TEMPLATE, raw_lbl):
    ...
    for b in range(B):
        ...
        # for tumor case
        if template_key in ['10_03', '10_07']:
            tumor_list = POST_TUMOR_DICT[template_key]
            for src, item in tumor_list:
...
data_dicts_file = os.path.join(ORGAN_DATASET_DIR,'data_dicts.json')

with open(data_dicts_file,'w') as file:
    json.dump(data_dicts_train,file,indent=4)
...
for index, batch in enumerate(train_loader):
    ...
    print(f'Start generating new label for {name}')
    y = generate_label(y, NUM_CLASS, name, TEMPLATE, y_raw)
    name = batch['name'][0].replace('label', 'post_label')
    print(f'label generated successfully for {name}')
    #print(name)
    post_dir = ORGAN_DATASET_DIR + '/'.join(name.split('/')[:-1]) #creating post_labelTr to save the preprocessed files. Postlabels.
