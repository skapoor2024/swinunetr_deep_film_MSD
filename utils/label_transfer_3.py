...
ORGAN_DATASET_DIR = '/blue/kgong/s.kapoor/language_guided_segmentation/CLIP-Driven-Universal-Model/'
...
class ToTemplatelabeld(MapTransform):
    ...
    def __call__(self, data: Mapping[Hashable, NdarrayOrTensor]) -> Dict[Hashable, NdarrayOrTensor]:
        ...
        #print(d['label'].keys())
        original_label_meta_tensor = d['label']
        
        
        new_label = self.totemplate(d['label'], TEMPLATE[template_key], tumor=TUMOR, tumor_list=tumor_list)

        new_meta_tensor = MetaTensor(new_label,affine = original_label_meta_tensor.affine)
        new_meta_tensor.meta.update(original_label_meta_tensor.meta)
        d['label'] = new_meta_tensor
        #print(d['label'].keys())
        return d
...
class RL_Splitd(MapTransform):
    ...
    def __call__(self, data: Mapping[Hashable, NdarrayOrTensor]) -> Dict[Hashable, NdarrayOrTensor]:
        ...
        if dataset_index in [5,8,13]:
            ...
            new_label = self.spliter(d['label'], [2], d['name'])
            # print(d['name'], np.unique(d['label']))
        elif dataset_index == 7:
            new_label = self.spliter(d['label'], [12], d['name'])
        elif dataset_index == 12:
            new_label = self.spliter(d['label'], [2, 16], d['name'])
        else:
            pass
