...
"""
Writing custom collaet function to hanlde batch size > 1 
"""

# def custom_collate(batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
#     collated_batch = {}
...
#     for key in keys:
#         # Collect all items for each key
#         key_items = [item[key] for item in batch if key in item]
        
#         # Handle different types of data
#         if isinstance(key_items[0], torch.Tensor):
#             collated_batch[key] = torch.stack(key_items, dim=0)
#         elif isinstance(key_items[0], list) and isinstance(key_items[0][0], torch.Tensor):
#             collated_batch[key] = [torch.stack([x for x in item_list], dim=0) for item_list in zip(*key_items)]
#         else:
...
def get_loader(args):
    ...
    if args.phase == 'train_val':
        ...
        val_sampler = DistributedSampler(dataset=val_dataset, even_divisible=True, shuffle=True) if args.dist else None
        val_loader = DataLoader(val_dataset, batch_size=args.val_batch_size, shuffle=False, num_workers=4, collate_fn=custom_nested_collate_2_val_test,sampler = val_sampler)
    ...
    if args.phase == 'validation':
        if args.cache_dataset:
            ...
            val_dataset = Dataset(data=data_dicts_val, transform=val_transforms)
        val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=4, collate_fn=custom_nested_collate_2_val_test)
    ...
    if args.phase == 'test':
        ...
        test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=4, collate_fn=custom_nested_collate_2_val_test)
...
def get_loader_without_gt(args):
    ...
    if args.cache_dataset:
        ...
        test_dataset = Dataset(data=data_dicts_test, transform=val_transforms)
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=4, collate_fn=custom_nested_collate_2)
    return test_loader, val_transforms

def get_loader_with_gt(args):
    ...
    for line in open(args.dataset_txt_path):
        name = line.strip().split()[1].split('.')[0]
        img_path = os.path.join(args.data_root_path,line.strip().split()[0])
        label_path = os.path.join(args.data_root_path,line.strip().split()[1])
        post_label_path = os.path.join(args.data_root_path,name.replace('label','post_label')+'.h5')
        val_img.append(img_path)
        val_label.append(label_path)
        val_post_label.append(post_label_path)
        val_name.append(name)

    data_dicts_val = [{'image': image, 'label': label, 'post_label': post_label, 'name': name}
                for image, label, post_label, name in zip(val_img, val_label, val_post_label, val_name)]
    print('val len {}'.format(len(data_dicts_val)))

    data_dicts_val_path = os.path.join(args.result_save_path,'data_dicts_val.json')

    with open(data_dicts_val_path,'w') as jfile:
        json.dump(data_dicts_val,jfile,indent = 4)
...
def get_loader_with_txtprompts(args):
    train_transforms = Compose(
        [
            LoadImageh5d(keys=["image", "label"]), #0
            AddChanneld(keys=["image", "label"]),
    ...
    for item in args.dataset_list:
        for line in open(args.data_txt_path + item +'_test.txt'):
            ...
            test_post_lbl.append(args.data_root_path + name.replace('label', 'post_label') + '.h5')
            test_name.append(name)
            test_prompts.append(get_key(name))
    data_dicts_test = [{'image': image, 'label': label, 'post_label': post_label, 'name': name, 'prompt': prompt}
                for image, label, post_label, name, prompt in zip(test_img, test_lbl, test_post_lbl, test_name, test_prompts)]
    print('test len {}'.format(len(data_dicts_test)))

    #we will give back both training and validation dataloader.
