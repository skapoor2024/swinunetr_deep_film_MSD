...
from FiLMSpatial import FiLMSpatial

class SwinUNETR_DEEP_FILMSpatial(nn.Module):
    ...
    def __init__(self,img_size,in_channels,out_channels,feature_size = 48,text_dim = 512,conditonal_vec = True,precomputed_prompt_path = 'dir',model_params=None,normalize = True,spatial_dims=3) -> None:
        ...
        d,h,w = img_size
        #add deep film spatial block
        spatial_dims = [
            (d * h * w) // (8 ** (i+2)) for i in range(4)
        ]
        
        # Create FiLMBlock modules
        self.deep_film_block = nn.ModuleList([
            FiLMSpatial(spatial_dim, text_dim) for spatial_dim in spatial_dims
        ])
    ...
    def forward(self,x,prompt = None,return_features = False):
        ...
        # torch.Size([6, 1, 96, 96, 96]) torch.Size([6, 48, 96, 96, 96]) torch.Size([6, 48, 48, 48, 48]) 
        # torch.Size([6, 96, 24, 24, 24]) torch.Size([6, 192, 16,16, 16]) torch.Size([6, 768, 8, 8, 8])
