...
class SwinUNETR_FILM(nn.Module):
    ...
    def __init__(self,img_size,in_channels,out_channels,feature_size = 48,text_dim = 512,conditonal_vec = True,precomputed_prompt_path = 'dir',model_params=None,normalize = True,spatial_dims=3) -> None:
        ...
        # self.output_block = nn.Conv3d(in_channels=feature_size,out_channels=out_channels,kernel_size=(1,1,1))
        # nn.init.xavier_uniform_(self.output_block.weight)

        #output block taken from monail.

        #self.output_block = UnetOutBlock(spatial_dims=spatial_dims,in_channels=feature_size,out_channels=out_channels)
    ...
    def forward(self,x,prompt = None,return_features = False):
        ...
        film_bottleneck = self.film_block(dec4,text_embs)

        dec3 = self.swin_unetr.decoder5(film_bottleneck, hidden_states_out[3])
